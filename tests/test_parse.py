...
def main() -> int:
    ...
    print("\n[17] 짧은 키워드가 단어 경계를 넘어 걸리지 않는지")
    from src.filters import _hits as _kw

    INC = ["유공", "포상", "표창", "스마트", "실증", "공개검증", "결과 발표", "디지털전환"]
    check(
        "'면세유 공급'이 '유공'으로 걸리지 않음",
        _kw("「농업용 면세유 공급 및 관리규정」 개정 알림", INC),
        [],
    )
    check(
        "진짜 유공 공고는 걸림",
        "유공" in _kw("「2026년 스마트제조혁신 유공」 포상 후보자 모집 공고", INC),
        True,
    )
    check(
        "긴 키워드는 띄어쓰기 무시가 유지됨 ('공개 검증'→공개검증)",
        "공개검증" in _kw("포상 후보자 사전공개 및 공개 검증", INC),
        True,
    )
    check(
        "'디지털 전환'도 디지털전환으로 걸림",
